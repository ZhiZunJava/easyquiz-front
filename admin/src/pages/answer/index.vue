    ...
    <div class="table-container">
      ...
      >
        <template #op="slotProps">
          <t-space>
            <t-link theme="primary" @click="toDOPaper(slotProps.row.id)"> 批改试卷 </t-link>
          </t-space>
        </template>
      </t-table>
    </div>
...
const toDOPaper = (id: any) => {
  const url = router.resolve({ path: '/answer/judge', query: { id } });
...
const fetchData = async () => {
  ...
  try {
    ...
    await examPaperAnswer.waitList(query).then((res) => {
      data.value = res.list;
