    ...
    <t-form ref="form" :data="queryParam" :label-width="80" colon @reset="resetData" @submit="searchData">
      <t-row>
        <t-col :span="10">
          <t-row :gutter="[24, 24]">
            <t-col :span="4">
              <t-form-item label="学科" name="subjectId">
                <t-select v-model="queryParam.subjectId" class="form-item-content" placeholder="请选择学科" clearable>
                  >
                  <t-option
    ...
    <div class="table-container">
      ...
      >
        <template #op="slotProps">
          <t-space>
            <t-link theme="primary" @click="toDOPaper(slotProps.row.id)"> 查看试卷 </t-link>
          </t-space>
        </template>
      </t-table>
    </div>
...
const COLUMNS: any = [
  ...
  },
  {
    ...
    width: 80,
    cell: (h: any, { row }: { row: any }) => {
      return `${row.userScore}分/${row.paperScore}分`;
    },
  },
  {
    title: '题目对错',
    ...
    width: 80,
    cell: (h: any, { row }: { row: any }) => {
      return `${row.questionCorrect}/${row.questionCount}`;
    },
  },
  {
    title: '耗时',
...
const toDOPaper = (id: any) => {
  const url = router.resolve({ path: '/answer/completeRead', query: { id } });
...
const fetchData = async () => {
  ...
  try {
    ...
    await examPaperAnswer.completeList(query).then((res) => {
      data.value = res.list;
